...
/**
 * This program performs image segmentation using a simple region growing
 * algorithm. The user needs to select seed points in each region that should be
 * detected (at least one seed point required in the region). A region is
 * defined as the adjacent pixels with very similar grayscale values.
 * 
 * Once the algorithm is run each region is shown in a grayscale value
 * corresponding to the seed that originated that region. All pixels that do not
 * belong to any of the regions are shown in dark red color.
 * 
 * The user selects the seeds by clicking on the image with a mouse. The program
 * responds to the following key strokes r - resets the image to the original x
 * - select random seed points d - run the region growing algorithm (using depth
 * first search) and show the results b - run the region growing algorithm
 * (using breadth first search) and show the results
 * 
 * 
...
public class RegionGrowing extends PApplet {
	...
	private String[] allFileNames = { "ChessBoard.png", // 0
			"BW_Pattern.png", // 1
			"Gray_Pattern.jpg", // 2
			"Lighthouse.jpg", // 3
	...
	String[] testFileNames = { "ChessBoard.png", "BW_Pattern.png", "Gray_Pattern.jpg", "Lighthouse.jpg",
			"brain_image_MRI.jpg", "cameraman.jpg" };
	...
	int[][] testSeeds = { { 3821, 54226, 55445, 3617, 26029, 26060, 33014, 33466 },
			{ 7684, 19715, 24973, 42713, 31122 }, { 78291, 68464, 83523, 76846, 55080, 46308, 37577, 45298 },
			{ 42923, 81143, 185800 },
	...
	/**
	 * This method is ignored when the program is run as an applet. It is
	 * provided to allow for command line runs as an application.
	 * 
	 * @param args
	 */
	...
	/**
	 * Setup to program to either run in the interactive mode or in the test
	 * mode. This method is run once to setup the window and initialize the data
	 * fields.
	 */
	public void setup() {
		if (RUN_TESTS_DFS)
	...
	/**
	 * Runs the actual applications. The method is runs in a continuous loop
	 * (loop provided in other code). It provides the graphical user interface
	 * for the interactive mode of the program.
	 */
	public void draw() {
	...
	/**
	 * Handles mouse click event by adding a seed corresponding to the x and y
	 ...
	 * 
	 * @param pixels
	 *            the current pixels array
	 * @param imagePixelsCopy
	 *            the original pixels array
	 */
	 ...
	 * 
	 * @param pixels
	 *            the pixels of the current image
	 */
	 ...
	 * 
	 * @param x
	 *            coordinate of the pixel to be set
	 * @param y
	 *            y coordinate of the pixel to be set
	 * @param grayLevel
	 *            gray level to be assigned to the pixel
	 * @param pixels
	 *            array of pixels in which a pixel is modified
	 */
	...
	/**
	 * Retrieves the value of a specified pixel from the pixels array. Assumes
	 * that all three channels (R, G, B) have the same values.
	 * 
	 * @param x
	...
	/**
	 * Uses a depth first search approach to grow a region around a specified
	 * seed. All pixels added to the region have gray level values that fall
	 * within a +/- threshold of the gray level value of the seed. The threshold
	 * is computed as a standard deviation of the pixels in the 11x11
	 ...
	 * @param value
	 *            gray level to assign to the pixels in the generated region
	 * @param pixels
	 *            array of current pixels to which the region is added
	 * @param imagePixelsCopy
	 *            array of original pixels based on which the region is
	 *            calculated
	 */
	public void getRegionDFS(int x, int y, int value, int[] pixels, int[] imagePixelsCopy) {
		int px, py, index;
		// compute the threshold based on the standard deviation of the
		// grayscale values
		...
		// TO DO: replace the line below with your own implementation of the
		// stack
			...
			if (px != 0 && px != img.width - 1 && py != 0 && py != img.height - 1) {
				for (int nx = px - 1; nx <= px + 1; nx++) {
					for (int ny = py - 1; ny <= py + 1; ny++) {
						pixDifference = Math.abs(
								red(imagePixelsCopy[y * img.width + x]) - red(imagePixelsCopy[ny * img.width + nx]));
	...
	/**
	 * Uses a breadth first search approach to grow a region around a specified
	 * seed. All pixels added to the region have gray level values that fall
	 * within a +/- threshold of the gray level value of the seed. The threshold
	 * is computed as a standard deviation of the pixels in the 11x11
	 * neighborhood of the seed.
	...
	public void runInteractive() {
		...
		// print an error message if image was not loaded correctly and
		// terminate
		if (img == null) {
	...
	/**
	 * Setup and run the test mode of the program. The depth first search
	 * approach to region growing is applied to several predefined images
	 * (testFileNames array) using the predefined seeds (testSeeds array). The
	 * resulting images are saved to the disk with names matching the input
	 * files, followed by "_DFS_output" followed by the original file extension.
	 */
	public void runTestsDFS() {
		// generate the directory name to which the files are saved
		String outDir = (System.getProperty("user.dir")).replace("bin", "src") + System.getProperty("file.separator");
	...
	/**
	 * Setup and run the test mode of the program. The breadth first search
	 * approach to region growing is applied to several predefined images
	 * (testFileNames array) using the predefined seeds (testSeeds array). The
	 * resulting images are saved to the disk with names matching the input
	 * files, followed by "_BFS_output" followed by the original file extension.
	 */
